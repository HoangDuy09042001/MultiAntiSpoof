...
def check_image(image):
    ...
    if width / height != 3 / 4:
        return False
    return True
# Function to reduce glare in the image using OpenCV
def reduce_glare(image):
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    hsv[:,:,2] = np.clip(hsv[:,:,2] - 50, 0, 255)  # Reduce value (brightness) by 50
...
def predict():
    ...
    image = reduce_glare(image)
    reduced_glare_image_path = '/home/hungha/AI_365/Cruel_Summer/reduced_glare_image.jpg'
    cv2.imwrite(reduced_glare_image_path, image)
    image = cv2.imread(reduced_glare_image_path)
    ...
    label = ((int(np.argmax(prediction))==1) or (abs(prediction[0][2] - prediction[0][1]) < 0.4)) 
    # label = (int(np.argmax(prediction))==1) 
    print('prediction[0][2]: ', prediction[0][2],' ','prediction[0][1]: ',prediction[0][1], ' ', 'abs(prediction[0][2] - prediction[0][1]): ',abs(prediction[0][2] - prediction[0][1]) )
